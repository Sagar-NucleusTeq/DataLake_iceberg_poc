{{ config(
    alias='policy_search_iceberg_dup',
...
SELECT
  ...
  {{ cleanse_text_field(source('fast','policy_search'),'policyprefix') }} AS policy_prefix,
  {{ cleanse_text_field(source('fast','policy_search'),'policynumber') }} AS policy_number,
