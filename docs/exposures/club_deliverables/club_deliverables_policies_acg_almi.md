...
{% docs club_deliverables_exposure_policies_acg_almi %}
{% raw %}

# Policies for ACG (ALMI) - Club Deliverable
...
file that is supported by redshift. This exposure is provided to ACG (ALMI) and contains
policy information for the following clubs: ['006', '012', '014', '020', '047', '049', '069', '113', '242', '243', '270'].
...
Today's folder location: /ACG (ALMI)/policy/V1/YYYY/MM/DD/policies_ACG (ALMI).parquet
...
| Filters:                 | ACG (ALMI) only - filtering done before sent to ACG (ALMI) |
