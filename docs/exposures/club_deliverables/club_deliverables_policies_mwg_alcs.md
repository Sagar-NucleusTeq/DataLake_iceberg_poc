...
{% docs club_deliverables_exposure_policies_mwg_alcs %}
{% raw %}

# Policies for MWG (ALCS) - Club Deliverable
...
file that is supported by redshift. This exposure is provided to MWG (ALCS) and contains
policy information for the following clubs: ['002', '005', '066', '255', '950'].
...
Today's folder location: /MWG (ALCS)/policy/V1/YYYY/MM/DD/policies_MWG (ALCS).parquet
...
| Filters:                 | MWG (ALCS) only - filtering done before sent to MWG (ALCS) |
